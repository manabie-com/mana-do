...
export function updateTodo(value: string, todoId: string): UpdateTodoAction {
  return {
    type: UPDATE_TODO,
    payload: {
      todoId,
...
export interface TodoActive {
  type: typeof TODO_ACTIVE;
  payload: string;
}
export function todoActive(status: string): TodoActive {
  return {
    type: TODO_ACTIVE,
    payload: status,
  };
...
export function todoComplete(status: string): TodoCompleted {
  return {
    type: TODO_COMPLETE,
    payload: status,
  };

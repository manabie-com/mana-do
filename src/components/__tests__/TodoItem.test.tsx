...
describe("TodoItem component", () => {
  describe("Todo item with Active status", () => {
    ...
    it("should contain active color", () => {
      expect(screen.queryByTestId("todo-item")).toHaveClass(
        "todo-color-active"
      );
    });
  });
  ...
  describe("Todo item when click checkbox", () => {
    ...
    it("Fire update status handler", () => {
      const todoItemCheckBoxEle: HTMLInputElement =
        screen.getByTestId("todo-item-checkbox");
  ...
  describe("Todo item double click to edit", () => {
    ...
    it("should not editable from start", () => {
      expect(screen.queryByTestId("todo-item-content")).toHaveAttribute(
        "readOnly"
      );
    });

    it("should editable after double click", () => {
      const todoItemContentEle: HTMLInputElement =
        screen.getByTestId("todo-item-content");
      fireEvent.doubleClick(todoItemContentEle);
      fireEvent.change(todoItemContentEle, {
        target: { value: "new content" },
      });

      expect(screen.queryByTestId("todo-item-content")).not.toHaveAttribute(
        "readOnly"
      );

...
import { AppActions, createTodo, deleteAllTodos, deleteTodo, setTodos, toggleAllTodos, updateTodoContent, updateTodoStatus } from '../../store/actions';
...
describe('reducer', () => {
    ...
    test('should handle CREATE_TODO', async () => {
        ...
        expect(state().todos[1].content).toBe(todo2)
    })
    ...
    test('should handle UPDATE_TODO_CONTENT', async () => {
        ...
        act(() => {
            dispatch(updateTodoContent(resp.id, updateTodo1))
        })
    })
